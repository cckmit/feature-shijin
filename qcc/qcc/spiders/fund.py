...
def read_invalid_fund_name(self, is_spider):
    ...
    pages = es_utils.get_page(ESIndex.INVEST_CHINA_FUND, page_size=-1, queries=queries, show_fields=['fund_name', 'used_name', 'last_updated', 'securities_fund_md5'])
    for page in pages:
        last_updated = None
...
def writer_product_file(self, fund_name, fund_no, register_date, record_date, fund_type, trustee, status, fund_manager, source):
    spider_wormtime = self.date_utils.get_timestamp()
    securities_fund_md5 = self.md5_utils.get_md5(data=fund_name + register_date + record_date + fund_type + trustee + status + fund_manager)
    type = '新增'
...
def parse_manager(self, content_dict, meta):
    ...
    '''
    writer_manager_file(self, fund_name, fund_manager_en, register_no, record_date, register_date, registered_address,office_address,registered_capital,
                        paid_in_capital, business_nature, proportion, orgniztion_type, last_updated, meta['source'], type)
...
def writer_file(self, source, redis_dict, fund_name, spider_wormtime, type):
    ...
    self.file_utils.write_file(file_name=local_file_path, data_type='a', content=json.dumps(redis_obj).encode('utf-8').decode('unicode_escape'))

def writer_manager_file(self, fund_name, fund_manager_en, register_no, record_date, register_date, registered_address,
                        office_address, registered_capital, paid_in_capital, business_nature, proportion, orgniztion_type, last_updated, source, type):
    spider_wormtime = self.date_utils.get_timestamp()
    redis_dict = {}
