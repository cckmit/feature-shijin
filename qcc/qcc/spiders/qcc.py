...
from qcc.utils import es_utils, file_utils
...
def parse_base_info(self, scrapy, name, results):
    ...
    results_es = self.es_utils.get_page(ESIndex.BUSINESS_INFO, queries=Query(QueryType.EQ, 'credit_id', credit_id), page_index=1, show_fields=['base_info_md5', 'shareholder_info_md5', 'main_staff_md5', 'company_change'])
    ...
    yield scrapy.Request(company_change_url, callback=self.parse,meta={'name': name, 'credit_id': credit_id,
                'company_change_last_date': company_change_last_date, 'company_change_es': company_change_es}, headers=self.qcc_utils.get_qcc_token_headers(), priority=100)
    ...
    yield scrapy.Request(shareholder_info_url, callback=self.parse,
                         meta={'name': name, 'credit_id': credit_id, 'unit': unit, 'page_index': 1,
                               ...
                               'shareholder_info_md5': get_es_md5(results_es, 'shareholder_info_md5')}, headers=self.qcc_utils.get_qcc_token_headers(), priority=100)
    ...
    yield scrapy.Request(main_staff_url, callback=self.parse,
                         meta={'name': name, 'credit_id': credit_id, 'page_index': 1,
                               'page_size': PAGEOPS.PAGE_SIZE,
                               'main_staff_md5': get_es_md5(results_es, 'main_staff_md5')}, headers=self.qcc_utils.get_qcc_token_headers(), priority=100)
# 数据翻页操作
def page_ops(self, spider_url, page_size, page_index, meta, scrapy, log_info):
    for page_index in range(page_index + 1, page_index + 2):
        ...
        yield scrapy.Request(url, callback=self.parse, meta=meta, headers=self.qcc_utils.get_qcc_token_headers(), priority=100)
...
def parse_eci_employee(self, response, scrapy, name, page_index, page_size, results):
    ...
    if page_index == total_pages:
        ...
        yield from page_ops(self, QCCAPI.MAIN_STAFF + '?key=' + QCCUtils.APPKEY + '&searchKey=' + name, page_size, page_index, meta, scrapy,  f'公司名称：{name} ,‘主要人员’ 添加待采集页数：{page_index+1}')
...
def parse_eci_partner(self, response, scrapy, name, page_index, page_size, results):
    ...
    if page_index == total_pages:
        ...
        yield from page_ops(self, QCCAPI.SHAREHOLDER_INFO + '?key=' + QCCUtils.APPKEY + '&searchKey=' + name, page_size, page_index, meta, scrapy,  f'公司名称：{name} ,‘股东信息’ 添加待采集页数：{page_index+1}')
