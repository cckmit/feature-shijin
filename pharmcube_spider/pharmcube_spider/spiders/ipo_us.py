...
from pharmcube_spider import const, run_env
    ...
    def parse(self, response):
        ...
        if 'source' in meta and 'orange_patent' == meta['source']:
            ...
            if len(exclusivity_list) > 0 or len(patent_list) > 0:
                ...
                self.file_utils.write_file(file_name=const.STORE_PATH + ESIndex.DRUG_US_ORANGE + '.txt', data_type='a',
                                           content=json.dumps(patent_obj).encode('utf-8').decode('unicode_escape'))
        ...
        if 'source' in meta and 'orange' == meta['source']:
            ...
            for content_element in content_elements.items():
                ...
                for split in splits:
                    ...
                    if len(split_doc('a')) > 0:
                        content_dict['href'] = 'https://www.accessdata.fda.gov/scripts/cder/ob/' + split_doc('a').attr(
                            'href')
                        continue
            ...
            self.file_utils.write_file(file_name=const.STORE_PATH + ESIndex.DRUG_US_ORANGE + '.txt', data_type='a',
                                       content=json.dumps(base_info_obj).encode('utf-8').decode('unicode_escape'))
        ...
        if 'source' in meta and 'us' == meta['source']:  # 美国库
            ...
            pages = es_utils.get_page('drug_us_drugs', page_size=-1,
                                      queries=Query(QueryType.EQ, 'application_num', application_num, ),
                                      show_fields=['original_approvals', 'supplement', 'label_arr', 'md5'])
