...
from pharmcube_spider.const import MongoTables, ESIndex
from pharmcube_spider.utils import pdf_utils, es_utils, qiniu_utils, common_utils
...
from pharmcube_spider.utils.send_email_utils import SendEmail
...
class MedliveSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if 'baidu' in spider_url:
            ...
            pages = es_utils.get_page(ESIndex.DRUG_IPO, queries=Query(QueryType.NE, 'drug_manual_medlive_url', None), page_index=-1, show_fields=['approval_num'])
            if None != pages:
                for page in pages:
        ...
        if 'medlive' in spider_url:
            ...
            doc = pq(str(left_elements.html()).replace('<u>','').replace('</u>',''))
            spider_content = ''
            ...
            for title_element in title_elements.items():
                ...
                value_next_elements = title_element.next()
                value = value_next_elements.text()
                if '药品名称' == title:
                ...
                if '批准文号' == title:
                    if value_next_elements('img').size() > 0:
                        self.send_email_dict[spider_url] = qiniu_url
                    medlive_approval_num_str = value_next_elements.html()
                ...
                if '进口药品注册证号' == title:
                    if value_next_elements('img').size() > 0:
                        self.send_email_dict[spider_url] = qiniu_url
                    medlive_import_approval_num_str = value_next_elements.html()
    ...
    def close(spider, reason):
        send_medlive_content = ''
        if len(spider.send_email_dict) >0:
            for medlive_url in spider.send_email_dict.keys():
                qiniu_url = spider.send_email_dict.get(medlive_url)
                send_medlive_content += "<tr><td>" + medlive_url + "</td><td>" + qiniu_url + "</td><td>" + spider.date_utils.defined_format_time(
                    timestamp=spider.date_utils.get_timestamp(), format='%Y-%m-%d') + "</td></tr>"

            send_content = "<table border=\"1\"><tr><th>医脉通网址</th><th>七牛云网址</th><th>spider_wormtime</th>"+send_medlive_content+"</tr></table>"
            query = {'send_project_list.send_project_name': 'medlive'}
            receiver = common_utils.get_send_email_receiver(query=query)
            spider.send_email_utils.send_email_content(content=send_content, receiver=receiver, subject='需人工处理-医脉通说明书的文号是图片格式')
...
def check_data(self, medlive_approval_num_list, medlive_import_approval_num_list, spider_url, qiniu_url, medlive_inn_name, medlive_drug_name_en, medlive_manufacturer, approval_num_set):
    ...
    if len(is_complete_dict) ==0:
        queries_cn = Query.queries(Query(QueryType.EQ, 'spider_drug_name', medlive_inn_name),
                                Query(QueryType.EQ, 'spider_manufacture', medlive_manufacturer), )
        update_es_data(self, queries_cn, spider_url, qiniu_url)

        queries_en = Query.queries(Query(QueryType.EQ, 'spider_drug_name_en', medlive_drug_name_en),
                                Query(QueryType.EQ, 'spider_manufacture_en', medlive_manufacturer), )
        update_es_data(self, queries_en, spider_url, qiniu_url)
...
def update_es_data(self, queries, spider_url, qiniu_url):
    pages = self.es_utils.get_page(const.ESIndex.DRUG_IPO, queries=queries, show_fields=['approval_num', 'spider_drug_name','spider_drug_name_en'])
    if None != pages:
        is_override_es = False
        for page in pages:
            if 'pharmcube' not in qiniu_url:
                logging.info(f'当前批准文号对应的说明书上传到七牛云有问题，被过滤：{spider_url}')
                continue
            esid = page['esid']
            approval_num = page['approval_num']
            if 'spider_drug_name' not in page or 'spider_drug_name_en' not in page:
                continue
            es_dict = {}
            es_dict['esid'] = esid
            es_dict['drug_manual_medlive_url'] = qiniu_url+f'?{self.date_utils.get_timestamp()}'
            logging.info(f'------- 更新 {const.ESIndex.DRUG_IPO} 药品说明书链接 -------{esid} {approval_num}')
...
def check_mongo_data(self, id_set, approval_num_set):
    mongo_data_list = self.mongo_utils.find_all(coll_name=MongoTables.MEDLIVE_DATA)
    for mongo_data in mongo_data_list:
        ...
        check_data(self, medlive_approval_num_list, medlive_import_approval_num_list, url, qiniu_url, medlive_inn_name,
                   medlive_drug_name_en, medlive_manufacturer, approval_num_set)