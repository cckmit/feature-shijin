...
#本地 es 地址
# __PRIVATE_DICT = {"es_host": ("127.0.0.1",),"port":"9200"}
#
#测试环境
# __PRIVATE_DICT = {"es_host": ("123.56.187.45",),'port':"9299"}
# __PRIVATE_DICT = {"es_host":("123.56.187.45"),"port":"9001"}
# __PRIVATE_DICT = {"es_host":("10.66.205.210",),"port":"9299"}
#__PRIVATE_DICT = {"es_host":("123.56.187.45",),"port":"9299"}
#线上 es 地址
__PRIVATE_DICT = {"es_host": ("10.27.223.106","10.27.223.106","10.29.130.193","10.144.112.79","172.17.108.73","172.17.108.74",),"port":"9299"}
...
def __get_client(host=None):
	if not host:
		...
		port1=__PRIVATE_DICT["port"]
	if not host in __d_host:
		print("current es host:{}".format(host))
		es = Elasticsearch(hosts=[{"host": ip, "port": port1} for ip in host], sniff_on_start=False,
...
def insert_or_replace(index: str, d: dict, *, es_dict=False, refresh=RefreshPolicy.IMMEDIATE, host=None):
	...
	if es_dict:
		...
		d2 = d
		esid = __get_esid(d)
	print('__get_client:',esid)
	es = __get_client(host=host)
	# index(self, index, body, doc_type=None, id=None, params=None, headers=None)
	return __is_success(es.index(index=index, body=d2,doc_type=index, id=esid, refresh=refresh.value))
...
def get_all(index, *, queries=None, show_fields: [str, tuple, list] = None,
			 orders: [str, dict, OrderedDict] = None, esid_field: [bool, str] = True, search_values: list = None, result=None,
			 score_field: str = None,host=None):
	es = __get_client(host=host)
	index = __get_index_new(index)
	query = __get_query(queries, show_fields=show_fields)
	sorts = __get_sorts(orders)
	...
	fetct_source = __get_fetct_source(show_fields)
	show_fields = __get_show_fields(show_fields)
	search_response = es.search(index=index, body=body, _source=fetct_source,_source_include=show_fields,explain=False)
	result = __get_list_from_search_response(search_response, fetct_source, esid_field, score_field, ls=result)
	return result
