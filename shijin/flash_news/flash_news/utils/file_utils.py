...
# def read_xls(file_path, file_name,read_rows):
#     xls_data = get_data(unicode(file_path+file_name), )
#     return xls_data


# rows: [{}]
def write_xls(file_path, file_name, rows ):
    ...
    for row in rows:
        ...
        if len(data) % 65530 ==0:
            ...
            open(f'{file_path}v{times}_{file_name}', 'wb',).write(data.xls)
            data = []
            times = times+1
    data = tablib.Dataset(*data, headers=header)
    open(file_path+file_name, 'wb', ).write(data.xls)
