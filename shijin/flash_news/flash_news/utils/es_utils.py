...
def __update_core(index: str, d: dict, *, upsert=False, refresh=RefreshPolicy.IMMEDIATE, retry_on_conflict=False):
	esid = __get_esid(d)
	es = __get_client()
	retry = retry_on_conflict and 3 or 0
	a = es.update(index, index, esid, body=__get_update_body(d, upsert), refresh=refresh.value, retry_on_conflict=retry)
...
def get_exist(index: str, esid: str) -> bool:
	es = __get_client()
	index = __get_index_new(index)
...
def oper_func_by_query(index, func_ls, *, queries=None, orders: [str, dict, OrderedDict] = None,
	...
	if size < 0 or size > scroll_size:
		...
		search_response = es.search(index=index, body=body, size=scroll_size, _source=fetct_source, _source_include=show_fields, scroll=__ES_SCROLL_TIME_5M)
		scroll_id = __get_es_scroll_id(search_response)
		result = __get_list_from_search_response(search_response, fetct_source, esid_field, score_field)
		if result:
			...
			if print_run_msg:
				current_size = len(result)
				print_datetime_now("索引：{},总条数:{},当前次数:{},已获取条数:{}".format(index, total, 0, current_size))
			# 判断是否中止
			...
			if not is_interrupt:
				if total > scroll_size:
					...
					for i in range(1, times + 1):
						...
						current_size += len(result)
						if print_run_msg:
							print_datetime_now("索引：{},总条数:{},当前次数:{}/{},已获取条数:{}".format(index, total, i, times, current_size))
		...
		if result:
			if print_run_msg:
				current_size = len(result)
				print_datetime_now("索引：{},当前次数:{},已获取总条数:{}".format(index, 0, current_size))
			func_ls(result)
	if print_run_msg:
